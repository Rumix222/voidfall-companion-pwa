...
#Evolution 22 : Objectifs galactiques (§3.3) — Lot 1, calcul automatique des conditions "exploit"
✅ Traité en partie (13/09/2026) — retour utilisateur : "Passons à l'implémentation des gains des objectifs galactiques à résoudre / choisir à chaque fin de cycle", décision actée avant de coder : "calcul automatique quand possible". Inventaire du catalogue AVANT de coder (30 événements, 65 lignes d'objectifs : 35 "exploit", 24 "multiplicateur", 6 "formule") — chantier bien plus gros que prévu, découpé en lots comme les Technologies en leur temps.
Nouveau module PUR `js/objectifsService.js` (`evaluerCondition`/`evaluerObjectifs`, 16 tests) : calcule si la CONDITION d'une ligne "exploit" est remplie, à partir d'un `contexte` assemblé par `strategieService.js` (popup 'phase_evaluation', section "Objectifs galactiques" — jusqu'ici un simple texte "Non automatisé"). Couvre 28 clés de condition sur les 31 utilisées par les lignes "exploit" (30/35 lignes automatisables) — `SecteurService.obtenirAgregatsInfluenceSecteursPurs` étendue avec les agrégats manquants (population/défense-base stellaire/guildes par secteur/secteurs possédés Purs+Corrompus).
Application du gain : reste MANUELLE pour l'instant (affichage "Condition remplie ✅ / non remplie ❌ / non calculable ❓"), SAUF le cas le plus simple (mode "unique", 1 seul gain "influence" sans "par"/"formule" — 5 lignes sur 35) : ajouté automatiquement à l'Influence à la validation, même mécanisme que les points de Programme.
⚠️ Hors périmètre de ce lot (à faire plus tard) :
- Les 5 lignes "exploit" restantes (`emplacements_guilde_vides_max`, `secteurs_avec_guildes_specifiques_min`, `focus_preferes_absents_de_defausse` — donnée jamais trackée, pas de pioche/défausse Focus modélisée —, `jetons_catastrophe_plateau_crise` — Plateau Crise "light" ne suit pas ce jeton —, `revenu_credit_min` — calcul de Revenu asynchrone pas encore branché dans le contexte).
- Les 24 lignes "multiplicateur" (gain répété N fois selon un compteur, parfois plafonné) et les 6 lignes "formule" (gain calculé) — types entièrement différents, pas touchés par ce Lot 1.
