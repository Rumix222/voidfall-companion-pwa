...
|---|---|---|---|
| `maisons.json` | `maisons` | 14 | Les 14 maisons jouables — technologies de départ, contenu fixe du Secteur-Mère/Maison déchue, champ `effet` = texte intégral de la Compétence de Maison (`null` pour les 4 maisons "de base" Valnis/Cortozaar/Belitan/Dunlork ; **affichage seul** sur Plat. maison, `GameService.formatMaison_`/`index.html:renderMaisonEffet_` — les 10 compétences ne sont pas automatisées). `secteurMereInstallationMax` (override `null` par défaut, sinon remplace `typesSecteur.json` "secteur_mere".`nombreInstallationMax` — voir `SecteurService.maxInstallationSecteurMere`), `secteurMerePeutEtreCorrompu` (`false` par défaut — le Secteur-Mère standard est immunisé à la Corruption), `secteurMereEntretienBonus` (`0` par défaut, Entretien fixe additionnel tant que le Secteur-Mère appartient au joueur) : 3 exceptions connues au Secteur-Mère standard — Astoran (`secteurMereInstallationMax`: 3, 2 Chantiers Navals constructibles en plus de celui imprimé), Marqualos et Novaris (`secteurMerePeutEtreCorrompu`: true ; Novaris a aussi `secteurMereEntretienBonus`: 1 et 2 Chantiers Navals imprimés) — les 11 autres maisons utilisent le Secteur-Mère standard. |
...
| `choisirTechnologieAvancee` | `(partieId, slot, nomTechnologie)` | Une des 4 Technologies avancées (slot 0-3), cycle 1 uniquement, rejette les doublons — pool = les 8 des maisons déchues UNIQUEMENT (`technologiesAdversesToutes_`), jamais la Technologie de départ du joueur (retour utilisateur 19/09/2026, corrige une erreur de règle du 13/09/2026) |
| `obtenirTechnologiesAvanceesGroupes` | `(partie)` | **Pure.** `{toutes, groupeA, groupeB, actif}` — `actif` = noms améliorables au cycle en cours parmi les 8 des maisons déchues. La Technologie de départ est HORS de ce mécanisme : `#check-amelioree-depart` (index.html) se déverrouille dès le cycle 1, indépendamment de `.actif` — seule Technologie améliorable aussi tôt |
...
- `INFLUENCE_DEPART = 10`, `GLOIRE_DEPART = [2, null, null, null, null]` — valeur du 1er jeton (2) surchargeable par Origine via `gloireDepart_`/`GLOIRE_DEPART_ORIGINE_` (Fenrax/Surveillance centrale : 3).
- `obtenirProgrammeOrigineAleatoire_` / `PROGRAMME_ORIGINE_ALEATOIRE_` : Origines dont l'effet fait piocher au hasard 1 carte Programme d'un type donné en main à la création de partie (Thegwyn/Matrice neuronale → Force, Fenrax/Surveillance centrale → Soutien, Nervo/Vaisseaux-Arches → Richesse, Shiveus/Chambres de décontamination → Soutien, Kradmor/Purificateur → Domination) — ajoutée à `plateauMaison.programmesEnMain`.
- `jetonCommerceDepart_` / `JETON_COMMERCE_DEPART_ORIGINE_` : Origines dont l'effet donne directement 1+ jeton(s) Commerce à la création de partie (Valnis/Nacelles : 1, Kradmor/Purificateur : 1, Marqualos/Drones autonomes : 2, Marqualos/Nexus de commerce : 1) — SIMPLE incrément de `plateauMaison.jetonCommerce` (longueur du tableau, valeurs `'disponible'`), sans résoudre de récompense (contrairement à la clé FocusEngine `gagner_commerce`, qui incrémente CE MÊME compteur ET fait en plus choisir 1 des 6 `BONUS_COMMERCE`, retour utilisateur 20/09/2026 — voir focusEngine.js resoudreCle_).
- `programmeDepartCorrompu_` / `RETIRER_CORRUPTION_PROGRAMME_DEPART_ORIGINE_` : Origines qui retirent la Corruption par défaut du dernier emplacement Programme (index 3 de `programmesUtilisesParDefaut_`, Corrompu dès la mise en place pour TOUTE maison — règle du livret) — Kradmor/Purificateur. AUCUN choix (contrairement à `appliquerDeplacerCorruptionOrigine` ci-dessous, qui déplace une Corruption existante vers une autre cible) : résolu entièrement dans `creerPartie` (pur, aucune popup) — ajuste `plateauMaison.programmesUtilises[3].corrompu` ET `corruptionMaison` (0 au lieu de 1) en cohérence.
- `GameService.appliquerJetonPrimeOrigine` / `JETON_PRIME_DEPART_ORIGINE_` / `jetonPrimeDepartOrigine_` : Origines dont l'effet donne directement N jeton(s) Prime à la création de partie (Yarvek/Hyperpropulsion : 3, Yarvek/Transports tactiques : 1, Marqualos/Drones autonomes : 1, Marqualos/Nexus de commerce : 1) — CONTRAIREMENT à jetonCommerceDepart_ ci-dessus, chaque jeton Prime fait choisir au joueur 1 des 11 faces réelles (`FocusEngine.resoudreGainJetonsPrime_`, même popup qu'un gain en cours de partie) : nécessite `demanderChoix` (DOM), donc appelée depuis `index.html afficherPartieCreee` (juste après `GameService.creerPartie`, AVANT `ouvrirPartie`) plutôt que depuis `creerPartie` lui-même (pur). Garde-fou `plateauMaison.jetonPrimeOrigineApplique` (bool) contre un double appel — initialisé à `false` par `creerPartie` pour toute maison, mis à `true` par `appliquerJetonPrimeOrigine` même en cas d'annulation de la popup (mise en place ponctuelle, pas rejouable).
- `GameService.appliquerDeplacerCorruptionOrigine` / `DEPLACER_CORRUPTION_DEPART_ORIGINE_` / `deplacerCorruptionDepartOrigine_` : Origines dont l'effet déplace 1 Corruption à la création de partie (Shiveus/Chambres de décontamination) — réutilise la clé FocusEngine `deplacer_corruption` déjà outillée (même popup Source/Destination à 2 étapes qu'ailleurs, `strategieService.js`). Appelée depuis `index.html afficherPartieCreee` APRÈS `ouvrirPartie` (contrairement à `appliquerJetonPrimeOrigine` ci-dessus) : la popup `deplacer_corruption` lit `StrategieService.partieAffichee` (calcul des sources/destinations éligibles), qui n'est renseigné qu'après un premier `StrategieService.afficher` (inclus dans `ouvrirPartie`) — un 2e `ouvrirPartie` ré-affiche les écrans si la Corruption a bougé. ⚠️ Contrairement à `appliquerJetonPrimeOrigine`/`appliquerRecompenseRefuge`, ne réutilise PAS le `plateauMaison` capturé avant l'appel à `FocusEngine.resoudreEffet` pour son écriture finale : `deplacer_corruption` persiste directement en base depuis sa propre popup (jamais via `resultatEffet.mutations`), donc `appliquerDeplacerCorruptionOrigine` relit `plateauMaison` à neuf après résolution avant d'y poser son propre garde-fou, pour ne jamais écraser cette écriture avec une valeur périmée.
...
### 4.10 `js/objectifsService.js` — Objectifs galactiques (Lots 1+2+3 : exploit + multiplicateur + formule)

**Rôle** : évalue une ligne d'Objectif galactique (moitié droite de la
carte Événement galactique, catalogue `evenements.json`,
`objectifs.blocs[].lignes[]`) — sa CONDITION booléenne ("exploit"), son
COMPTEUR d'occurrences ("multiplicateur") ou sa VALEUR calculée
("formule"). Module PUR (aucun DOM/DB) — `contexte` est assemblé par
l'appelant (`strategieService.js`/`construireContexteObjectifs_`, popup
...
— y compris le détail Fermiers/Ingénieurs/Mineurs par secteur, Lot 2 —/
`secteursPossedes[]`), plus `cubeActif`/`cubesSecteurPurTotal`/`revenus`
(Lot 3 — `StrategieService.calculerNiveauxProduction_` +
`calculerProductionAvecBonusTechnologie_`, le Revenu RÉEL des 5
ressources, pas le niveau brut ni le stock en réserve).
...
voir `GameService.GLOIRE_DEPART`) — `jetons_gloire_min`/`_exact`
utilisaient `.length` directement (toujours 5) au lieu du nombre réel de
